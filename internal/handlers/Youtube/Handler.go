...
func (h Handler) GetYoutubePlaylistBySpotifyLink(youtubeLink string) (*domain.Playlist, error) {
	playlist, err := h.spotifyService.GetSpotifyPlaylistById(youtubeLink)
	if err != nil {
		return nil, err
	}
	return playlist, nil
...
func (h Handler) FillYoutubePlaylist(playlist domain.Playlist, AuthToken string) (*domain.Playlist, error) {
	YoutubePlaylist, err := h.youtubeService.CreateYoutubePlaylist(playlist, AuthToken)
	if err != nil {
		return nil, err
	}
